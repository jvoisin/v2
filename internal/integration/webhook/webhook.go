...
func (c *Client) makeRequest(eventType string, payload any) error {
	...
	response, err := httputil.DoJSONRequestWithBody(http.MethodPost, c.webhookURL, requestBody, map[string]string{
		"X-Miniflux-Signature":  crypto.GenerateSHA256Hmac(c.webhookSecret, requestBody),
		"X-Miniflux-Event-Type": eventType,
	})
	if err != nil {
